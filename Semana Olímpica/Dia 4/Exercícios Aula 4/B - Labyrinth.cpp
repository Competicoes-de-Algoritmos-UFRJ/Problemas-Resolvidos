...
vector<vector<int>> dist; //dist[i][j] := distance traversed from A to position (i,j)
vector<vector<char>> lastMove; //lastMove[i][j] := move used to discover position (i,j) in BFS, either R, D, L, U
vector<int> di = {0, 1, 0, -1}, dj = {1, 0, -1, 0}; //variations of coordinates in moes R, D, L, U. (i+di[0], j+dj[0]) = (i, j+1) => move to the right
