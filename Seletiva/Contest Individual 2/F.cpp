...
signed main(){
    ...
    while(!qw.empty()){
        ...
        for(pair<int,int> cc: dir){
            int xc = cc.first + x, yc = cc.second + y;
            if(xc >= n || xc < 0 || yc >= m || yc < 0) continue;
            
            if(vis[xc][yc]) continue;
            
            char c = grid[x][y], nxt = grid[xc][yc];
            if(nxt != mp[c]) continue;

            qw.push(make_pair(xc,yc));
            vis[xc][yc] = 1;
        }
    }
